...
class RoadSegmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RoadSegment
        fields = [
            'id', 'points', 'contractor_id', 'contractor_name', 'contractor_email', 
            'contractor_phone', 'road_creation_date', 'warranty_period', 'money_sanctioned',
...
class RoadSegmentCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = RoadSegment
        fields = [
            'points', 'contractor_id', 'contractor_name', 'contractor_email', 
            'contractor_phone', 'road_creation_date', 'warranty_period', 'money_sanctioned'
        ]
    ...
    def validate_contractor_email(self, value):
        """Validate contractor email format"""
        import re
    ...
    def validate_contractor_phone(self, value):
    ...
    def validate_money_sanctioned(self, value):
        """Validate money sanctioned amount"""
        if value <= 0:
            raise serializers.ValidationError("Money sanctioned must be greater than 0")
        if value > 99999999999.99:  # 12 digits total, 2 decimal places
            raise serializers.ValidationError("Money sanctioned amount is too large")
        return value

    def validate_contractor_id(self, value):
        """Validate contractor ID format"""
        if len(value.strip()) < 3:
            raise serializers.ValidationError("Contractor ID must be at least 3 characters long")
        if len(value.strip()) > 100:
            raise serializers.ValidationError("Contractor ID cannot exceed 100 characters")
